...
(() => {
	...
	var rspackExports = (__webpack_require__.aE = hasSymbol
		? Symbol("rspack exports")
		: "__webpack_exports__");
	var rspackError = hasSymbol ? Symbol("rspack error") : "__rspack_error";
	var rspackDone = hasSymbol ? Symbol("rspack done") : "__rspack_done";
	var rspackDefer = (__webpack_require__.zS = hasSymbol
		? Symbol("rspack defer")
		: "__rspack_defer");
		...
		deps.map((dep) => {
			if (null !== dep && "object" == typeof dep) {
				if (!dep[rspackQueues] && dep[rspackDefer]) {
					...
					dep = {
						then(callback) {
							Promise.all(asyncDeps.map(__webpack_require__)).then(() =>
								callback(d),
							);
						},
					};
	...
	__webpack_require__.a = (module, body, hasAwait) => {
		...
		var handle = (deps) => {
			...
			var promise = new Promise((resolve) => {
				...
				currentDeps.map(
					(dep) => dep[rspackDefer] || dep[rspackQueues](fnQueue),
				);
			});
			return fn.r ? promise : getResult();
		};
		var done = (err) => (
			err ? reject((promise[rspackError] = err)) : outerResolve(exports),
			resolveQueue(queue),
			(promise[rspackDone] = true)
		);
		body(handle, done);
