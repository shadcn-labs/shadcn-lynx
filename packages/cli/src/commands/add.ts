...
import { DEFAULT_REGISTRY_URL, readConfigFile } from '../utils/config.js';
...
export async function add(
  ...
  const registryUrl =
    process.env.REGISTRY_URL ?? config.registry ?? DEFAULT_REGISTRY_URL;
  ...
  // Write files
  const uiBasePath = resolveAliasPath(
    cwd,
    config.aliases?.ui ?? 'src/components/ui',
  );
  ...
  for (const item of allItems) {
    ...
    for (const file of item.files) {
      ...
      if (file.type === 'registry:ui') {
        ...
        const hooksPath = resolveAliasPath(
          cwd,
          config.aliases?.hooks ?? 'src/hooks',
        );
